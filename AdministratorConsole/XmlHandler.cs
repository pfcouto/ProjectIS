...
namespace AdministratorConsole
{
    class XmlHandler
    {
        public static void OutputTXml(string filename, List<Transaction> transactions)
        {
            ...
            {
                root.AppendChild(createTransaction(doc, transaction.Id.ToString(), transaction.Type.ToString(), transaction.VCard, transaction.Payment_reference, transaction.Date.ToString("dd-MM-yyyy HH:mm:ss"), transaction.Value.ToString() + "€"));
            }
